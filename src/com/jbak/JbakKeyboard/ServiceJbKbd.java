 ...
 * out as appropriate.
 */
public class ServiceJbKbd extends InputMethodService implements
		KeyboardView.OnKeyboardActionListener
{
	...
	public static final int STATE_TEMP_SEL = 0x0000002;
	...
	public View onCreateCandidatesView()
	{
		 ...
		 * mCandidateView.setService(this); return mCandidateView;
		 */// return null;
		return null;
	}
	@Override
			...
			int candidatesEnd)
	{
		...
		if (st.has(m_state, STATE_TEMP_SEL))
		{
			st.log("tempsel:cur(" + newSelStart + "," + newSelEnd + ") end="
					+ m_SelEnd);
			m_state = st.rem(m_state, STATE_TEMP_SEL);
			m_state |= STATE_SEL;
			return;
		}
		if (st.has(m_state, STATE_SEL))
		{
			st.log("sel:cur(" + newSelStart + "," + newSelEnd + ") end="
					+ m_SelEnd);
			m_state = st.rem(m_state, STATE_SEL);
	...
	public boolean onKeyDown(int keyCode, KeyEvent event)
	{
		if (KeySetActivity.inst != null
				&& KeySetActivity.inst.onHardwareKey(event))
			return true;
	...
	public void onKey(int primaryCode, int[] keyCodes)
	{
		...
		if (st.has(st.kv().m_state, JbKbdView.STATE_SOUNDS))
		{
			...
			if(primaryCode==KeyEvent.KEYCODE_DPAD_LEFT) sound = SoundEffectConstants.NAVIGATION_LEFT;
			if(primaryCode==KeyEvent.KEYCODE_DPAD_RIGHT) sound = SoundEffectConstants.NAVIGATION_RIGHT;
			if(primaryCode==KeyEvent.KEYCODE_DPAD_UP) sound = SoundEffectConstants.NAVIGATION_UP;
			if(primaryCode==KeyEvent.KEYCODE_DPAD_DOWN) sound = SoundEffectConstants.NAVIGATION_DOWN;
	...
	private void handleCharacter(int primaryCode, int[] keyCodes)
	{
		...
		{
			JbKbdView.inst.m_state = st.rem(JbKbdView.inst.m_state,
					JbKbdView.STATE_TEMP_SHIFT);
			JbKbdView.inst.setShifted(st.has(JbKbdView.inst.m_state,
					JbKbdView.STATE_CAPS_LOCK));
